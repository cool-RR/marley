...
class ModelFreeLearningPolicy(QPolicy):
    ...
    def get_next_policy(self, story: Story) -> Policy:
        ...
        if self.training_counter + 1 == self.training_period: # It's training time!
            ...
            clone_kwargs['serialized_models'] = self.train_one_model()
    ...
    def train_one_model(self) -> Tuple[bytes]:
        serialized_models = list(self.serialized_models)
        random_index = random.randint(0, len(serialized_models) - 1)
        other_index = (random_index + 1) % len(serialized_models)
        # (It's the index of another model if there's >=2, otherwise it's the same model.)
        serialized_models[random_index] = self.model_jockey.clone_model_and_train(
            serialized_models[random_index],
            lambda model: self._train_model(model, other_model=self.models[other_index])
        )
        return tuple(serialized_models)
